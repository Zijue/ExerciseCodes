...
export function patch(oldVnode, vnode) {
    // console.log(vnode);
    if (!oldVnode) { // 组件的渲染：组件渲染时patch(this.$el, vnode)，此时this.$el为空
        return createEle(vnode); // 创建一个真实节点返回给this.$el
    }
...
function createComponent(vnode) { // 用于创建组件的真实节点，成功返回true，失败false
    let i = vnode.props;
    if ((i = i.hook) && (i = i.init)) { // 组件有init方法，那就调用init。在创建组件的虚拟节点时添加的
        i(vnode); // new Ctor().$mount();  成功后，vnode.componentInstance.$el上就会挂载组件的真实dom节点
    }
...
export function createEle(vnode) { // 创建真实的节点
    const { tag, props, children, text } = vnode;
    if (typeof tag === 'string') {
        if (createComponent(vnode)) { // 创建组件的真实节点，就看能不能创建出来
            return vnode.componentInstance.$el;
        } else {
            vnode.el = document.createElement(tag); // 将创建的真实dom和虚拟dom映射在一起方便后续更新和复用
