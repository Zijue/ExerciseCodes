...
let ws = new WriteStream(path.resolve(__dirname, 'writeSample.txt'), {
    flags: 'w',
    encoding: 'utf8',
    // mode: 0o666,
    emitClose: true,
    start: 0,
    highWaterMark: 3, // 我期望使用多少个字节完成写入操作，如果超出后write的返回值会变成false， 返回false 可以用于判断，告知用户不要在写入了，再写入只能放到内存中，占用内存
});
let index = 0;
